...
uint32_t PadcapReadReg24(uint8_t ui8Channel, uint8_t ui8Register)
{
	uint16_t output;
	msg_t status = RDY_OK;
	txbuf[0] = ui8Register;
...
void servicePadCaps()
{
	...
	// ui8Val[0] = PadcapReadReg8(0x70, 0);
	// ui8Val[1] = PadcapReadReg8(0x70, 1);
	// ui8Val[2] = PadcapReadReg8(0x70, 2);
	// ui8Val[3] = PadcapReadReg8(0x70, 3);
	//
	// ui32CapTouched =  (
	// 					( (ui8Val[0] << 24) && 0xFF000000) |
	// 					( (ui8Val[1] << 16) && 0x00FF0000) |
	// 					( (ui8Val[2] <<  8) && 0x0000FF00) |
	// 					( (ui8Val[3]      ) && 0x000000FF)
	// 				);
	// touch event
	if (eventType == 0)
