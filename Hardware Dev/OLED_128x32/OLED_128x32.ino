...
#define OLED_RESET 4
Adafruit_SSD1306 display(OLED_RESET); // tbd-s
...
void setup()   {
  ...
  // will display splash screen from buffer on first display
  // display.display();
  // delay(2000);
  // init done
...
void testDrawSteps()
{
    for (int16_t row =0; row < 5; row++)
    {
        ...
        {
          display.drawCircle(2+i*8, row*8 + 4 , 2, WHITE);
        }
    }
    display.display();
}
...
void testHorzScroll(void)
{
  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(10,0);
  display.clearDisplay();
  display.println("Attack / Decay / Sustain / Release");
  display.display();
  delay(1);
